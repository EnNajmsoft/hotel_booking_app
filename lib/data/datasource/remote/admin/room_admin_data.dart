...
class RoomAdminData {
  Crud crud;
  RoomAdminData(this.crud);

  getroomData(String hotelid) async {
    var response = await crud.postData(AppLink.linkVieweRoom, {"hotelid" : hotelid});
    return response.fold((l) => l, (r) => r);
  }

  editeActivedata(String roomactive , roomid) async {
    var sresponse = await crud.postData(AppLink.linkEditActiveRoom, {"roomactive": roomactive ,"roomid": roomid});
    return sresponse.fold((l) => l, (r) => r);
  }

  deletroomdata(roomid , imagename) async {
    var sresponse = await crud
        .postData(AppLink.linkdeletRoom, { "roomid": roomid, "imagename": imagename});
    return sresponse.fold((l) => l, (r) => r);
  }

  editroomdata( 
   roomid, roomname,
   roomnamear, roomprice,
   roomdescont, roomnote,
  ...
  addroomdatawitfile(
      hotelid, roomname, roomnamear, roomprice, roomdescont, roomnote, myfile) async {
